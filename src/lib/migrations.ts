...
// The project's schema migration, as a list of idempotent statements
// (2026-09-20: moved here from src/app/api/admin/migrate/route.ts so the
// same list can be applied from the Actions runner via
// `npx tsx scripts/run-job.ts migrate` — the route still exists and calls
// this). There is no migration framework by design: every statement is
// CREATE/ALTER ... IF NOT EXISTS (or the DO-block equivalent for
// constraints), the whole list is applied on every run, and it no-ops
// where already applied. Append; never edit a shipped statement in a way
// that changes what it does on a fresh database.
export const MIGRATION_STATEMENTS = [
  ...
  // 2026-09-20: the wide daily feature snapshot — see countryFeatureDaily's
  // doc comment in schema.ts.
  sql`CREATE TABLE IF NOT EXISTS country_feature_daily (
    id SERIAL PRIMARY KEY,
    country TEXT NOT NULL,
    ...
    momentum_direction SMALLINT NOT NULL,
    event_count INTEGER NOT NULL,
    pillar_geopolitical_security DOUBLE PRECISION NOT NULL DEFAULT 0,
    pillar_political_governance DOUBLE PRECISION NOT NULL DEFAULT 0,
    pillar_climate_environment DOUBLE PRECISION NOT NULL DEFAULT 0,
    pillar_natural_biological_hazards DOUBLE PRECISION NOT NULL DEFAULT 0,
    pillar_human_social DOUBLE PRECISION NOT NULL DEFAULT 0,
    pillar_infrastructure_connectivity DOUBLE PRECISION NOT NULL DEFAULT 0,
    pillar_supply_chain_resource DOUBLE PRECISION NOT NULL DEFAULT 0,
    pillar_cyber_technology DOUBLE PRECISION NOT NULL DEFAULT 0,
    events_24h INTEGER NOT NULL DEFAULT 0,
    events_7d INTEGER NOT NULL DEFAULT 0,
    severity_mean_24h DOUBLE PRECISION,
    severity_max_24h SMALLINT,
    severity_mean_7d DOUBLE PRECISION,
    distinct_sources_7d INTEGER NOT NULL DEFAULT 0,
    distinct_families_7d INTEGER NOT NULL DEFAULT 0,
    telegram_share_7d DOUBLE PRECISION,
    gdelt_share_7d DOUBLE PRECISION,
    novelty_items_24h INTEGER NOT NULL DEFAULT 0,
    novel_share_24h DOUBLE PRECISION,
    novelty_mean_distance_24h DOUBLE PRECISION,
    aircraft_military_latest INTEGER,
    aircraft_military_z14 DOUBLE PRECISION,
    aircraft_commercial_latest INTEGER,
    aircraft_commercial_z14 DOUBLE PRECISION,
    gps_jamming_latest INTEGER,
    gps_jamming_z14 DOUBLE PRECISION,
    days_since_anomaly DOUBLE PRECISION,
...
export async function applyMigrations(): Promise<{ statementsApplied: number }> {
  ...
  return { statementsApplied: MIGRATION_STATEMENTS.length };
}
