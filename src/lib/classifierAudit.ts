...
- The EXACT source "telegram:presstv" (and ONLY that source — no other Telegram channel, including other Iranian state-linked ones like telegram:iribnews, telegram:defapress_ir, telegram:sepah_pasdaran, telegram:Nournews_ir) is scoped to axis-of-resistance conflict content (widened 2026-09-10 from an Iran-only rule, user request): Iran, or Yemen/Houthi, Lebanon/Hezbollah, and Iraq/PMF or other Iran-aligned militias being attacked, attacking, or threatening others. A presstv item about Yemen/Houthi or Iraq/PMF conflict action is now correctly IN scope, not excluded — do not flag it as over-included, and do not flag its ABSENCE as a miss either if it's not there yet, this is a recent change. What still stays OUT of scope for presstv specifically (both exclusions below are DELIBERATE, do not flag either as a miss or as over-application):
  (a) Any Gaza/Palestine/West Bank/Hamas mention at all (user request, 2026-09-06), even if Iran or another axis actor is also named.
  (b) Direct Iran-US military conflict — IRGC vs the US Navy, US strikes on Iranian tankers, Iran retaliating against US bases, an Iranian official condemning a US attack, etc. — in EITHER direction (user request, 2026-09-10): this exact bilateral story is presstv's single highest-volume beat and also the most heavily wire-covered US-Iran story globally, already captured from every angle by this app's other sources. presstv's real, distinct value is axis-of-resistance content mainstream wire coverage under-serves (Yemen, Iraq, Lebanon), not re-telling the one US-Iran story already told everywhere.
This restriction does NOT apply to any other source: real conflict content from other Iranian-affiliated or state-linked channels about Israel-Palestine, US-Iran conflict, Yemen, Saudi Arabia, Iraq, etc. is normal, in-scope content there — do not invent or assume a similar restriction exists for them. A live audit run (2026-09-08) found Gemini incorrectly over-generalizing this presstv-only rule to other Iranian channels; be precise about which exact source string this applies to.`;
