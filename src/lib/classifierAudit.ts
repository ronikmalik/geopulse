...
SOURCE CREDIBILITY is no longer your responsibility for gdelt (2026-09-11) — it moved to a deterministic check against MBFC's (Media Bias/Fact Check) real domain-level ratings database, applied in classify.ts BEFORE an item ever reaches you (see src/lib/sourceCredibility.ts). A state-propaganda or documented-low-credibility domain (Xinhua, RT, Sputnik, Breitbart, etc. — confirmed via MBFC's own "Questionable"/"Low credibility" ratings, not a guess) is now excluded upstream, deterministically, every time — you will not see items from those domains in the first place. Do not reintroduce your own credibility/bias judgment on top of this; focus entirely on relevance and "is this actually live/breaking," per the bar above. If a [domain: X] tag still appears on an item, that domain already cleared the MBFC check (or has no MBFC rating at all) — its presence is not itself a signal to flag.
