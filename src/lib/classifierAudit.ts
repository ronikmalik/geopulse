...
//
// SUPERSEDED same-day by articleTitleFetch.ts (2026-09-10): gdelt items no
// longer display the synthesized CAMEO description this section describes
// — every gdelt-sourced RawItem now comes from drainPendingGdeltTitles,
// which fetches the REAL title/snippet from the source page and discards
// the templated description entirely (see gdeltBulk.ts's own header
// comment). The actor-coherence/generic-institution scrutiny above is
// still correct and still needed (it's about GDELT's underlying event
// tagging, not the display text), but "not written by a journalist"/
// "carries none of a real headline's own signals" no longer describes
// what Gemini is actually shown — GDELT_BULK_GUIDANCE's own wording was
// corrected 2026-09-11 (user report: analysis/editorial pieces and scope
// creep getting through) to stop telling Gemini the title text itself is
// synthetic, which was undermining exactly the real-headline judgment
// (DELIBERATE_EXCLUSIONS' opinion/analysis/retrospective bullet) that
// text needs applied to it just as much as RSS gets.
const GDELT_BULK_GUIDANCE = `GDELT gets a DELIBERATELY STRICTER bar than every other source in this prompt (2026-09-11, explicit user instruction: "far higher credibility and relevancy bar for the gdelt inclusions... i am tired of this... even if it means lowering the gdelt approval rate a [lot] more"). Everywhere else in this app's calibration, the default is to favor recall — "if you are unsure, include it." For gdelt specifically, that default is REVERSED: if you are unsure whether a gdelt item clears the bar below, the correct answer is to flag it as a false positive (kept-item prompt) or NOT recover it (dropped-item prompt). A materially lower approval rate for gdelt than for RSS/Telegram is the intended, correct outcome of this rule — it is not a sign that something is broken, and you should not compensate for it by being more lenient on borderline cases.

The bar: this app wants LIVE, BREAKING events — something that JUST HAPPENED and is being reported on AS a fresh happening, not:
- A periodic or statistical roundup — a tally of operations/incidents/casualties accumulated "over the past week/month/quarter," a "quarterly security update," or any piece whose real subject is a SUMMARY across a reporting period rather than one specific incident. Real example that should NOT have been kept: "Nigeria's security agencies engaged in 7,062 operations, killed 1,487 terrorists in two months" — real conflict vocabulary, real severity-sounding numbers, but the subject is a period-end tally, not something that just happened.
- A wire-service rehash or syndicated pickup of a story that's already several hours old, even when the specific headline wording is new (see the redundant-rehash bullet above — that same discipline applies here with extra weight for gdelt specifically).
- An institutional/government press-release-style report, statement, or briefing ABOUT a security situation, rather than a report OF an incident. Ask: is the article's subject "X happened just now," or "here is our summary/assessment/report of a situation"?
- Analysis, explainer, or retrospective framing (see the exclusions above) — apply that bar at least as strictly here as everywhere else, more strictly if anything, since gdelt items skew toward wire/institutional sourcing where this framing is common.
...
- SEPARATELY (2026-09-11, user instruction: "not just state media but also known notoriously highly biased/garbage (uncredible) sites") — regardless of any state affiliation and regardless of what country the story is about: flag any item whose domain is a widely-documented low-credibility source. This means a site with a real, known track record of fabricated stories or invented quotes, a hyperpartisan outlet that runs conspiracy content or unverified rumor as straight news, or a pure content-farm/clickbait operation with no real editorial or fact-checking process at all. This is a DIFFERENT failure mode from the geographic-mismatch bullet above — a low-credibility outlet is unreliable everywhere, including stories about its own country, not just stories outside its usual region. It is also a DIFFERENT axis from ordinary political lean: an outlet with a clear ideological slant but a genuine fact-checking record, named/accountable reporters, and a real editorial process is NOT what this targets — the test is documented factual unreliability and fabrication, not tone, opinion content elsewhere on the site, or a political viewpoint you may disagree with. Do not flag a mainstream outlet, a national paper of record, or a real wire service on credibility grounds just because it leans a particular direction editorially.
- Neither bullet above is a rule against non-Western, non-mainstream, or regional sources generally — a REGIONAL outlet covering ITS OWN region (a Nigerian paper on Nigeria, a Pakistani outlet on Pakistan, an Israeli paper on Israel-Palestine) is exactly the kind of credible, close-to-the-ground reporting this app wants; staying skeptical of that would be its own bias. A state outlet reporting on ITS OWN country's conflict (Russian state media on Russia-Ukraine, Iranian state media on Iran, subject to the presstv-specific rules above) is also a different, already-handled case, not what either credibility bullet targets.
- If you don't recognize the domain, or aren't confident it's genuinely state-controlled or has a real documented fabrication/low-credibility track record, do not flag on credibility grounds alone — both of these are targeted rules for domains you can specifically identify, not a general license to distrust unfamiliar sources.
...
- Opinion pieces, analysis, explainers, retrospectives ("years after...", "look back at...", anniversary pieces) — this applies whether or not the piece is explicitly labeled "analysis"/"opinion": an unsigned staff-written piece is just as excluded as a named commentator's if its actual content is background/context/implications rather than reporting a specific thing that is happening or just happened. Ask: is this reporting a concrete development, or reflecting on/explaining one that's already known?
- Redundant rehash coverage: a piece that only restates a development this feed has already reported, with NO new substantive fact (no updated casualty/damage figure, no new official statement/response, no new escalation or de-escalation, nothing that happened since the original report) — e.g. a "what we know so far" or "N days later" piece on an incident already covered that adds nothing beyond what was already known. Be CONSERVATIVE flagging this one specifically: only flag when you can point to the SPECIFIC prior report being rehashed and confirm nothing in this piece is new — a genuine follow-up (an updated toll, a named actor's new statement, a retaliation, a policy response, ANY new fact even a small one) is a real, separate, in-scope development and must NOT be flagged just because it concerns an incident already in the news. When you aren't sure whether something is genuinely new, do not flag it — this rule exists to catch pure rehashes, not to suppress a story's real developing updates, and favoring recall over precision here is the deliberate, existing calibration for this classifier (see the severity/inclusion bar elsewhere in this prompt).
